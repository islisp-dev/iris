...
(defmethod f ((x C) (y C) (z C))
                 ...
                 (f (create (class C))
                    (create (class C))
                    (create (class C10))))
(defmethod f ((x C) (y C) (z C10))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C 'C 'C10)
                 (f (create (class C))
                    (create (class C10))
                    (create (class C))))
(defmethod f ((x C) (y C10) (z C))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C 'C10 'C)
                 (f (create (class C10))
                    (create (class C))
                    (create (class C))))
(defmethod f ((x C10) (y C) (z C))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C10 'C 'C)
                 (f (create (class C))
                    (create (class C10))
                    (create (class C10))))
(defmethod f ((x C) (y C10) (z C10))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C 'C10 'C10)
                 (f (create (class C10))
                    (create (class C))
                    (create (class C10))))
(defmethod f ((x C10) (y C) (z C10))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C10 'C 'C10)
                 (f (create (class C10))
                    (create (class C10))
                    (create (class C))))
(defmethod f ((x C10) (y C10) (z C))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C10 'C10 'C)
                 (f (create (class C10))
                    (create (class C10))
                    (create (class C10))))
(defmethod f ((x C10) (y C10) (z C10))
