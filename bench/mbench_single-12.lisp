...
(defmethod f ((x C) (y C) (z C))
                 ...
                 (f (create (class C))
                    (create (class C))
                    (create (class C12))))
(defmethod f ((x C) (y C) (z C12))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C 'C 'C12)
                 (f (create (class C))
                    (create (class C12))
                    (create (class C))))
(defmethod f ((x C) (y C12) (z C))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C 'C12 'C)
                 (f (create (class C12))
                    (create (class C))
                    (create (class C))))
(defmethod f ((x C12) (y C) (z C))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C12 'C 'C)
                 (f (create (class C))
                    (create (class C12))
                    (create (class C12))))
(defmethod f ((x C) (y C12) (z C12))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C 'C12 'C12)
                 (f (create (class C12))
                    (create (class C))
                    (create (class C12))))
(defmethod f ((x C12) (y C) (z C12))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C12 'C 'C12)
                 (f (create (class C12))
                    (create (class C12))
                    (create (class C))))
(defmethod f ((x C12) (y C12) (z C))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C12 'C12 'C)
                 (f (create (class C12))
                    (create (class C12))
                    (create (class C12))))
(defmethod f ((x C12) (y C12) (z C12))
