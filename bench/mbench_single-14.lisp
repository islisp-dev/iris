...
(defmethod f ((x C) (y C) (z C))
                 ...
                 (f (create (class C))
                    (create (class C))
                    (create (class C14))))
(defmethod f ((x C) (y C) (z C14))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C 'C 'C14)
                 (f (create (class C))
                    (create (class C14))
                    (create (class C))))
(defmethod f ((x C) (y C14) (z C))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C 'C14 'C)
                 (f (create (class C14))
                    (create (class C))
                    (create (class C))))
(defmethod f ((x C14) (y C) (z C))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C14 'C 'C)
                 (f (create (class C))
                    (create (class C14))
                    (create (class C14))))
(defmethod f ((x C) (y C14) (z C14))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C 'C14 'C14)
                 (f (create (class C14))
                    (create (class C))
                    (create (class C14))))
(defmethod f ((x C14) (y C) (z C14))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C14 'C 'C14)
                 (f (create (class C14))
                    (create (class C14))
                    (create (class C))))
(defmethod f ((x C14) (y C14) (z C))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C14 'C14 'C)
                 (f (create (class C14))
                    (create (class C14))
                    (create (class C14))))
(defmethod f ((x C14) (y C14) (z C14))
