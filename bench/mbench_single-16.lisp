...
(defmethod f ((x C) (y C) (z C))
                 ...
                 (f (create (class C))
                    (create (class C))
                    (create (class C16))))
(defmethod f ((x C) (y C) (z C16))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C 'C 'C16)
                 (f (create (class C))
                    (create (class C16))
                    (create (class C))))
(defmethod f ((x C) (y C16) (z C))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C 'C16 'C)
                 (f (create (class C16))
                    (create (class C))
                    (create (class C))))
(defmethod f ((x C16) (y C) (z C))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C16 'C 'C)
                 (f (create (class C))
                    (create (class C16))
                    (create (class C16))))
(defmethod f ((x C) (y C16) (z C16))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C 'C16 'C16)
                 (f (create (class C16))
                    (create (class C))
                    (create (class C16))))
(defmethod f ((x C16) (y C) (z C16))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C16 'C 'C16)
                 (f (create (class C16))
                    (create (class C16))
                    (create (class C))))
(defmethod f ((x C16) (y C16) (z C))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C16 'C16 'C)
                 (f (create (class C16))
                    (create (class C16))
                    (create (class C16))))
(defmethod f ((x C16) (y C16) (z C16))
