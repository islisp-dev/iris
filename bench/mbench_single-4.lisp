...
(defmethod f ((x C) (y C) (z C))
                 ...
                 (f (create (class C))
                    (create (class C))
                    (create (class C4))))
(defmethod f ((x C) (y C) (z C4))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C 'C 'C4)
                 (f (create (class C))
                    (create (class C4))
                    (create (class C))))
(defmethod f ((x C) (y C4) (z C))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C 'C4 'C)
                 (f (create (class C4))
                    (create (class C))
                    (create (class C))))
(defmethod f ((x C4) (y C) (z C))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C4 'C 'C)
                 (f (create (class C))
                    (create (class C4))
                    (create (class C4))))
(defmethod f ((x C) (y C4) (z C4))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C 'C4 'C4)
                 (f (create (class C4))
                    (create (class C))
                    (create (class C4))))
(defmethod f ((x C4) (y C) (z C4))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C4 'C 'C4)
                 (f (create (class C4))
                    (create (class C4))
                    (create (class C))))
(defmethod f ((x C4) (y C4) (z C))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C4 'C4 'C)
                 (f (create (class C4))
                    (create (class C4))
                    (create (class C4))))
(defmethod f ((x C4) (y C4) (z C4))
