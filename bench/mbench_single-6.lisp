...
(defmethod f ((x C) (y C) (z C))
                 ...
                 (f (create (class C))
                    (create (class C))
                    (create (class C6))))
(defmethod f ((x C) (y C) (z C6))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C 'C 'C6)
                 (f (create (class C))
                    (create (class C6))
                    (create (class C))))
(defmethod f ((x C) (y C6) (z C))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C 'C6 'C)
                 (f (create (class C6))
                    (create (class C))
                    (create (class C))))
(defmethod f ((x C6) (y C) (z C))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C6 'C 'C)
                 (f (create (class C))
                    (create (class C6))
                    (create (class C6))))
(defmethod f ((x C) (y C6) (z C6))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C 'C6 'C6)
                 (f (create (class C6))
                    (create (class C))
                    (create (class C6))))
(defmethod f ((x C6) (y C) (z C6))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C6 'C 'C6)
                 (f (create (class C6))
                    (create (class C6))
                    (create (class C))))
(defmethod f ((x C6) (y C6) (z C))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C6 'C6 'C)
                 (f (create (class C6))
                    (create (class C6))
                    (create (class C6))))
(defmethod f ((x C6) (y C6) (z C6))
