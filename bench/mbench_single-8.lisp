...
(defmethod f ((x C) (y C) (z C))
                 ...
                 (f (create (class C))
                    (create (class C))
                    (create (class C8))))
(defmethod f ((x C) (y C) (z C8))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C 'C 'C8)
                 (f (create (class C))
                    (create (class C8))
                    (create (class C))))
(defmethod f ((x C) (y C8) (z C))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C 'C8 'C)
                 (f (create (class C8))
                    (create (class C))
                    (create (class C))))
(defmethod f ((x C8) (y C) (z C))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C8 'C 'C)
                 (f (create (class C))
                    (create (class C8))
                    (create (class C8))))
(defmethod f ((x C) (y C8) (z C8))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C 'C8 'C8)
                 (f (create (class C8))
                    (create (class C))
                    (create (class C8))))
(defmethod f ((x C8) (y C) (z C8))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C8 'C 'C8)
                 (f (create (class C8))
                    (create (class C8))
                    (create (class C))))
(defmethod f ((x C8) (y C8) (z C))
                 (format (standard-output) "call with ~A ~A ~A~%" 'C8 'C8 'C)
                 (f (create (class C8))
                    (create (class C8))
                    (create (class C8))))
(defmethod f ((x C8) (y C8) (z C8))
