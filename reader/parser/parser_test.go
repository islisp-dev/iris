...
func Test_parseAtom(t *testing.T) {
	...
	tests := []struct {
		...
		//
		{
			...
			want:      instance.Float(3.14),
			wantErr:   false,
		},
		{
			name:      "signed",
			arguments: arguments{"-5.0"},
			want:      instance.Float(-5.0),
			wantErr:   false,
		},
		{
			name:      "exponential",
			arguments: arguments{"-5.0E3"},
			want:      instance.Float(-5.0 * 1000),
			wantErr:   false,
		},
		{
			name:      "signed exponential",
			arguments: arguments{"5.0E-3"},
			want:      instance.Float(5.0 * 1.0 / 1000.0),
			wantErr:   false,
		},
		{
			name:      "without point",
