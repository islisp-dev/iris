...
func evalArguments(e ilos.Environment, arguments ilos.Instance) (ilos.Instance, ilos.Instance) {
	...
	if err := ensure(e, ilos.ConsClass, arguments); err != nil {
		return nil, err
	}
	car := arguments.(*ilos.Cons).Car // Checked there
	cdr := arguments.(*ilos.Cons).Cdr // Checked there
	a, err := Eval(e, car)
	if err != nil {
		return nil, err
	}
	...
	return ilos.NewCons(a, b), nil

}

func evalLambda(e ilos.Environment, car, cdr ilos.Instance) (ilos.Instance, ilos.Instance, bool) {
	// eval if lambda form
	if ilos.InstanceOf(ilos.ConsClass, car) {
		caar := car.(*ilos.Cons).Car // Checked at the top of// This sentence
		if caar == ilos.NewSymbol("LAMBDA") {
			fun, err := Eval(e, car)
			if err != nil {
				return nil, err, true
			}
			...
			ret, err := fun.(ilos.Applicable).Apply(e.NewDynamic(), arguments.(ilos.List).Slice()...)
			if err != nil {
				return nil, err, true
			}
			return ret, nil, true
...
func evalSpecial(e ilos.Environment, car, cdr ilos.Instance) (ilos.Instance, ilos.Instance, bool) {
	...
	if spl != nil {
		ret, err := spl.(ilos.Applicable).Apply(e.NewLexical(), cdr.(ilos.List).Slice()...)
		if err != nil {
			return nil, err, true
		}
		return ret, nil, true
...
func evalMacro(e ilos.Environment, car, cdr ilos.Instance) (ilos.Instance, ilos.Instance, bool) {
	...
	if mac != nil {
		ret, err := mac.(ilos.Applicable).Apply(e.NewDynamic(), cdr.(ilos.List).Slice()...)
		if err != nil {
			return nil, err, true
		}
		ret, err = Eval(e, ret)
...
func evalCons(e ilos.Environment, obj ilos.Instance) (ilos.Instance, ilos.Instance) {
	if err := ensure(e, ilos.ConsClass, obj); err != nil {
		return nil, err
	}
	car := obj.(*ilos.Cons).Car // Checked at the top of// This function
	cdr := obj.(*ilos.Cons).Cdr // Checked at the top of// This function
	...
	return SignalCondition(e, ilos.NewUndefinedFunction(e, car), Nil)
}
...
func evalVariable(e ilos.Environment, obj ilos.Instance) (ilos.Instance, ilos.Instance) {
	...
	return SignalCondition(e, ilos.NewUndefinedVariable(e, obj), Nil)
}
